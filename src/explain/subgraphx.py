...
class SubgraphX(nn.Module):
    ...
    def shapley(self, subgraph_nodes):
        ...
        for _ in range(self.shapley_steps):
            ...
            exclude_feat = self.node_feat * exclude_mask.unsqueeze(1).to(device)
            include_feat = self.node_feat * include_mask.unsqueeze(1).to(device)

            with torch.no_grad():
                exclude_probs = self.model(
                    self.graph, exclude_feat, self.edge_feat, **self.kwargs
                ).softmax(dim=-1)
                exclude_value = exclude_probs[:, self.target_class]
                include_probs = self.model(
                    self.graph, include_feat, self.edge_feat, **self.kwargs
                ).softmax(dim=-1)
    ...
    def explain_graph(self, graph, node_feat, edge_feat, target_class, **kwargs):
