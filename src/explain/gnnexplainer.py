...
class GNNExplainer(nn.Module):
    ...
    def explain_graph(self, graph, node_feat, edge_feat, **kwargs):
        ...
        with torch.no_grad():
            logits = self.model(graph, node_feat, edge_feat, **kwargs)
            pred_label = logits.argmax(dim=-1)

        feat_mask, edge_mask = self._init_masks(graph, node_feat)
        ...
        for _ in range(self.num_epochs):
            optimizer.zero_grad()
            h = node_feat * feat_mask.sigmoid()
            edge_wfeat = edge_feat * edge_mask[:, None].sigmoid()
            logits = self.model(graph, h, edge_wfeat, **kwargs)
