...
from data.electronegs import electronegs 
...
def smiles2graph(smiles, idxfunc=lambda x:x.GetIdx()):
    ...
    return torch.Tensor(fatoms), torch.Tensor(fbonds), torch.Tensor(atom_nb), torch.Tensor(bond_nb), torch.Tensor(num_nbs)
