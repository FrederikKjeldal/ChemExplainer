...
from gnn.utils import create_dirs, model_from_config, save_checkpoint, setup_logger
...
def train_model(config):
    ...
    # logging
    setup_logger(save_dir, log_name="train.log", debug=config["debug"])
    ...
    valid_loader = DataLoader(
        dataset=valid_data, batch_size=batch_size, collate_fn=collate_molgraphs
    )
    ...
    for epoch in range(1, max_epochs + 1):
        ...
        logging.info(
            f"epoch {epoch}/{max_epochs}, train_loss: {train_loss:.2f}, valid_loss: {valid_loss:.2f}, best valid score {best_valid_error:.2f}"
        )
